...
// takes 3 doubles representing y coordinate of triangle vertices
// returns the value of the largest one
double max_vertex( double y0, double y1, double y2 ) {
  double max = y0;
  if ( y1 > max )
    max = y1;
  if ( y2 > max )
...
// takes 3 doubles representing y coordinate of triangle vertices
// returns the vlaue of the smallest one
double min_vertex( double y0, double y1, double y2 ) {
  double min = y0;
  if ( y1 < min )
    min = y1;
  if ( y2 < min )
...
void scanline_convert( struct matrix *points, int i, screen s, color c, zbuffer zb ) {
  double max = max_vertex(points->m[1][i], points->m[1][i+1], points->m[1][i+2]);
  double min = min_vertex(points->m[1][i], points->m[1][i+1], points->m[1][i+2]);
  
  int j;
  int top_point_index;
  short top_filled = 0;
  int bottom_point_index;
  short bottom_filled = 0;
  int middle_point_index;
  short middle_filled = 0;

  for ( j = i; j < i+3; j++ ) {
    if ( points->m[1][j] == max && top_filled == 0) {
      top_point_index = j;
      top_filled += 1;
    }
    else if ( points->m[1][j] == min && bottom_filled == 0 ) {
      bottom_point_index = j;
      bottom_filled += 1;
    }
    else {
  ...
  //printf("%d: %f\t %d: %f\t %d: %f\n", top_point_index, polygons->m[1][top_point_index], middle_point_index, polygons->m[1][middle_point_index], bottom_point_index, polygons->m[1][bottom_point_index]);
  ...
  void draw_polygons( struct matrix *polygons, screen s, zbuffer zb, color c ) {
	...
	double magnitude = sqrt( pow(normal[0], 2) + pow(normal[1], 2) + pow(normal[2], 2) );
	for ( i = 0; i < 3; i++ ) 
	  normal[i] = normal[i] / magnitude;
	
	// determine ambient reflection
	// done in my_main.c
	// determine diffuse reflection
	int diffuse[3];
	double kd = 0.5; // constant for diffuse reflection
	for ( i = 0; i < lastsym; i++ ) { // for each light source
	  if ( symtab[i].type == SYM_LIGHT ) {

	    for ( j = 0; j < 3; j++ ) { // for each color
	      int light_color = symtab[i].s.l->c[j];
	      double dot_product = 0;
	      for ( k = 0; k < 3; k++ ) {// for xyz
		dot_product += (symtab[i].s.l->c[k] * normal[k]);
	      }
	      if ( dot_product < 0 )
		dot_product = 0;
	      printf("%f\n", dot_product);
	      diffuse[j] = light_color * dot_product * kd;
	    } // end for each color
