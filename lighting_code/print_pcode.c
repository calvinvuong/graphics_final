...
void print_pcode()
{
  ...
  for (i=0;i<lastop;i++)
	...
	{
	case LIGHT:
	  printf("Light: %s at: %6.2f %6.2f %6.2f with color: %6.2f %6.2f %6.2f",
		 op[i].op.light.p->name,
		 op[i].op.light.c[0], op[i].op.light.c[1], op[i].op.light.c[2],
		 op[i].op.light.p->s.l->l[0], op[i].op.light.p->s.l->l[1], op[i].op.light.p->s.l->l[2]);
	  break;
	case AMBIENT:
