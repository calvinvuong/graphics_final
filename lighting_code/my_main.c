...
/*======== void light_pass() ==========
Inputs: pointer to a color struct
Returns:

Goes through script, finds ambient values, sets ambient color struct to said values.
Finds light commands and updates symbol table
========================================*/
void light_pass(color * amb) {
  int i;
  double k_amb = 0.5; //constant of ambient reflection
  for (i=0;i<lastop;i++) {
    // set ambient colors
    if (op[i].opcode == AMBIENT) {
      (*amb).red = k_amb * op[i].op.ambient.c[0];
      (*amb).green = k_amb * op[i].op.ambient.c[1];
      (*amb).blue = k_amb * op[i].op.ambient.c[2];
    }
    
    // update symbol table with light structs
    else if (op[i].opcode == LIGHT) {
      struct light light_source;
      // color
      light_source.c[0] = op[i].op.light.p->s.l->l[0];
      light_source.c[1] = op[i].op.light.p->s.l->l[1];
      light_source.c[2] = op[i].op.light.p->s.l->l[2];

      //location
      double magnitude = sqrt( pow(op[i].op.light.c[0], 2) + pow(op[i].op.light.c[1], 2) + pow(op[i].op.light.c[2], 2) );
      light_source.l[0] = op[i].op.light.c[0] / magnitude;
      light_source.l[1] = op[i].op.light.c[1] / magnitude;
      light_source.l[2] = op[i].op.light.c[2] / magnitude;

      add_symbol(op[i].op.light.p->name, SYM_LIGHT, &light_source);
    }
  }
}
